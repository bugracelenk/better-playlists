...
class App extends Component {
	...
	componentDidMount(){
		...
		}).then(response => response.json()).then(data => this.setState({user: {name: data.id}}))
    ...
    return (
	    <div className="App">
		    {this.state.user ?
			<div>
		    		<h1 style={{color:defaultTextColor}}>{this.state.user && this.state.user.name}'s Playlists</h1>
